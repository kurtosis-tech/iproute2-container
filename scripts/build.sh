...
image_name_latest_version="${IMAGE_ORG_AND_REPO}:latest"
...
if "${do_build}"; then
    ...
    docker buildx build ${push_flag} --platform "${SUPPORTED_PLATFORMS}" -t "${image_name}" -t "${image_name_latest_version}" -f "${root_dirpath}/Dockerfile" "${root_dirpath}"
fi
